...
const CHARACTERS = [
    {
        ...
        camp: 'evil',
        campName: '邪恶阵营',
        // Target Vector: N, E, C, A, O (out of 40)
        vector: { N: 8, E: 32, C: 32, A: 8, O: 32 },
    ...
    },
    {
        ...
        camp: 'good',
        campName: '善良阵营',
        vector: { N: 15, E: 12, C: 38, A: 20, O: 35 },
        desc: '你的大脑是一台超级计算机。你看得透所有的套路，记得住所有的选票，但深知“木秀于林风必摧之”，选择在暗中指引光明。',
        details: '高精度的记忆与严密的逻辑（高C），同时拥有大局观（高O）。情绪上谨慎克制（中低N），发言克制，不当焦点（低E）。',
        blueprint: {
            C: '算无遗策。你是无情的票型记录仪，脑海中有着极其清晰的车队演进路线和逻辑闭环。',
            O: '多维视角。你能瞬间看破反派的伪装和反常规套路（Meta-game），对虚假信息的甄别能力极强。',
            E: '隐藏锋芒。你不屑于（或害怕）成为前台争吵的焦点，更倾向于通过精准的一票和不易察觉的微小发言来传递信息。',
            N: '谨慎克制。你内心虽然清晰，但时刻感受着被刺杀的压力，因此言行极其克制，不轻易上头。',
            A: '理智共情。你希望好人团队能赢，但绝不会因为某人的声泪俱下就放弃底层的票型逻辑。'
        },
    ...
    },
    {
        ...
        camp: 'evil',
        campName: '邪恶阵营',
        vector: { N: 10, E: 12, C: 34, A: 8, O: 12 },
        ...
        blueprint: {
            ...
            E: '冷眼旁观。你总是坐在角落，发言极其精简甚至故意划水，绝不主动吸引全场的火力。',
    ...
    },
    {
        ...
        camp: 'evil',
        campName: '邪恶阵营',
        vector: { N: 32, E: 32, C: 8, A: 10, O: 38 },
    ...
    },
    {
        ...
        camp: 'good',
        campName: '善良阵营',
        vector: { N: 35, E: 35, C: 12, A: 35, O: 18 },
        ...
        blueprint: {
            E: '大声疾呼。你拿到好人牌时底气十足，敢于冲锋陷阵，在讨论中声音最洪亮。',
...
function calculateAndDisplayResults() {
    ...
    // Normalize to 0-50 scale (since 5 questions * 10 max points)
    // rawScores coordinates are already in range 0 - 50.
    
    // Step 2: Compute Euclidean Distance for each of the 7 roles
    const matches = CHARACTERS.map(char => {
        const dist = Math.sqrt(
            Math.pow(rawScores.N - char.vector.N, 2) +
            Math.pow(rawScores.E - char.vector.E, 2) +
            Math.pow(rawScores.C - char.vector.C, 2) +
            Math.pow(rawScores.A - char.vector.A, 2) +
            Math.pow(rawScores.O - char.vector.O, 2)
        );
        
        // Convert to compatibility percentage: clamp(100 * (1 - dist / 75), 0, 100)
        // 75 divider gives a beautiful spread for the 50-max dimensions
        const similarity = Math.max(0, Math.min(100, Math.round(100 * (1 - dist / 75))));
